...
from legged_gym.envs.Go2_MoB.GO2_Trot.GO2_Stairs_config import GO2_Stairs_Cfg_Yu
...
class GO2_Stairs_Robot(BaseTask):
    def __init__(self, cfg: GO2_Stairs_Cfg_Yu, sim_params, physics_engine, sim_device, headless):
    ...
    def _reward_feet_clearance(self):#鼓励抬脚高度
        ...
        rew=torch.exp(-torch.sum(torch.abs(left_feet_height-target_height)*swing_mask[:,0].unsqueeze(1),dim=1)*50)
        # print(rew[0],torch.sum(torch.abs(left_feet_height-target_height),dim=1)[0])
        rew+=torch.exp(-torch.sum(torch.abs(right_feet_height-target_height)*swing_mask[:,1].unsqueeze(1),dim=1)*50)
